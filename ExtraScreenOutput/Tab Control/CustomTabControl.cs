	...
	public class CustomTabControl : TabControl
	{
		...
		private void DrawTabText(int index, Graphics graphics)
		{
			...
			if (this.SelectedIndex == index) {
				...
				if (this.TabPages[index].Enabled) {//location to edit drawn hilited text - dejis_edit
                    Font fntReg = new Font(this.Font.FontFamily, this.Font.Size, FontStyle.Regular);
					graphics.DrawString(this.TabPages[index].Text, fntReg, Brushes.White/*SystemBrushes.ControlText*/, tabBounds, this.GetStringFormat());
				} else {
                    Font fntBold = new Font(this.Font.FontFamily, this.Font.Size, FontStyle.Bold);
					graphics.DrawString(this.TabPages[index].Text, fntBold,SystemBrushes.ControlDark, tabBounds, this.GetStringFormat());
				}
			}
		}
		...
		private Brush GetTabBackgroundBrush(int index){
			...
			//	Capture the colours dependant on selection state of the tab
            Color dark = Color.Black;// Color.FromArgb(96, 134, 184);//Color.FromArgb(146,176,218);//FromArgb(207, 207, 207);
            Color light = Color.FromArgb(140, 140, 140);//(242, 242, 242);
			...
			if (this.SelectedIndex == index) {
				...
				//	Enable hot tracking
                light = Color.FromArgb(234, 246, 253);
                dark = Color.Gray;//Color.FromArgb(80,110,154);//(167, 217, 245);
			}
